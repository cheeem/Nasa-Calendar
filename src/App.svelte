...
,{
              ...
              copyright: `Javier CalderaMiguel Gracia`
  ...
  import svg_nasa from './assets/NASA.svg'
...
<main>
  ...
  <ol class="calendar">
    ...
    {#await process(query(`&start_date=${format_query_date(first_day)}&end_date=${format_query_date(last_day > new Date() ? new Date() : last_day)}`))}
    ...
    {:then apods}
      ...
      {#each apods as apod, i}
        ...
        <li class="day {apod && "can-hover"}" style={!i && `grid-column-start: ${first_day.getDay()}`}
